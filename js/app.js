...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <p>Avarage <i class="fas fa-star"></i> ${product.rating.rate} of ${product.rating.count} customer</p>
      <h2>Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="btn btn-success">add to cart</button>
      <button id="${product.id}" class="btn btn-secondary" onclick="getSingleProduct(${product.id})" >Details</button>
      </div>
  ...
  details.innerHTML = `
	<div class="row">
	    <div class="col-6 d-flex justify-content-center">
	        <img class="w-50 " src="${product.image}" alt="i">
        </div>
        <div class="col-6">
	        <h3>${product.title}</h3>
